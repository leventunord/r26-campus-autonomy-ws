...
HunterBaseRos::HunterBaseRos(std::string node_name)
  ...
  // zyq zjj 声明新参数
  this->declare_parameter("publish_tf", rclcpp::ParameterValue(true));  // 新增参数
...
void HunterBaseRos::LoadParameters() {
  ...
  // zyq zjj 获取新参数
  // 解释：从参数服务器中获取名为“publish_tf”的参数值，并将其存储在publish_tf_的成员变量中，
  //      如果参数服务器中没有该参数，则使用默认值true
  ...
  std::cout << "whether to publish odom->baselink tf: " << publish_tf_ << std::endl;
  std::cout << "----------------------------" << std::endl;
}
