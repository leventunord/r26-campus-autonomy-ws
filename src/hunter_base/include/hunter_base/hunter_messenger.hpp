...
class HunterMessenger {
 public:
  ...
  // zyq zjj 添加odom -> baselink tf树 发布判断
  void SetPublishTf(bool judge) { publish_tf_ = judge; }
 ...
 private:
  ...
  // zjj zyq add 控制是否发布odom -> baselink tf树
  ...
  void PublishOdometryToROS(const MotionStateMessage &msg, double dt) {
    ...
    // zjj zyq broadcaster tf maybe it need to be closed
    // to do add 判断 是否发布 tf
    if(publish_tf_) {
